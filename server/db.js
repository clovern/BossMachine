...
const getFromDatabaseByMinionId = (modelType, minionId) => {
  const model = findDataArrayByName(modelType);
  if (model === null) {
    return null;
  }
  //this will create a new array of all the work with a given minionId
  return model.data.filter((element) =>
     element.minionId === minionId
  );
}
...
const updateWorkInDatabase = (modelType, instance) => {
  const model = findDataArrayByName(modelType);
  if (model === null) {
    return null;
  }
  const instanceIndex = model.data.findIndex((element) => ((element.minionId === instance.minionId) && (element.id === instance.id)));
...
const deleteWorkFromDatabasebyId = (modelType, minionId, id) => {
  const model = findDataArrayByName(modelType);
  if (model === null) {
    return null;
  }
